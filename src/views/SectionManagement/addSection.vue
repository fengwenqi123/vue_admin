<template>
  <div class="add">
    ...
    <div v-if="mul" class="modSection">
      ...
      <div class="table">
        <el-button @click="addDialog" type="success">新增设备元件</el-button>
        <el-table
          :data="tableData"
          style="width: 100%">
          <el-table-column
...
<script>
  ...
  import {add} from '@/api/SectionManagement'
  ...
  export default {
    ...
    created() {
      if (this.mul.length !== 0) {
        ...
        this.alert.forEach((item, index) => {
          if (this.mul.alertType.toString() === item.value) {
            this.alertType = item.label
          }
        })
    ...
    methods: {
      ...
      submit() {
        ...
        add(this.id, this.code, this.name, this.rtmp, this.web, this.alertType, this.area, this.description, JSON.stringify(this.componentBeanList)).then(response => {
